class Resultado:
    def __init__(self):
        self.fechaBusqueda = None
        self.numeroRuc = None
        self.razonSocial = None
        self.tipoContribuyente = None
        self.fechaInscripcion = None
        self.fechaInicioActividades = None
        self.estadoContribuyente = None   
        self.condicionContribuyente = None
        self.domicilioFiscal = None
        self.sistemaEmisionComprobante = None      
        self.actividadComercioInterior = None
        self.sistemaContabilidad = None
        self.actividadesEconomicas = None 
        self.emisorElectronicoDesde = None
        self.comprobantesElectronicos = None
        self.afiliadoAlPLEDesde = None    
        self.padrones = None    
        self.importante = None                                                                                                                                              